...
Hypergraph<Index, Weight> readHMetisGraph(std::istream &s, std::size_t nNodes, std::size_t nEdges, bool hasEdgeWeights) {
  std::stringstream ss;
  HypergraphBuilder<Index, Weight> h(nNodes);
...
Problem<Index, Weight, Resource> readHMetis(std::istream &s) {
  Problem<Index, Weight, Resource> ret;
  ...
  ret.hypergraph = readHMetisGraph<Index, Weight>(s, nNodes, nEdges, hasEdgeWeights);
  ret.demands = readHMetisResources<Resource>(s, nNodes, hasNodeWeights);
  return ret;
}

template<class Index, class Weight, class Resource>
void writeHMetis(const Problem<Index, Weight, Resource> &pb, std::ostream &s) {
  s << "% HGR (hMetis) file generated by Minipart" << std::endl;

  const Hypergraph<Index, Weight> &h = pb.hypergraph;
